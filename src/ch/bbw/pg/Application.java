...
public class Application {
    ...
    public static void main(String[] args) throws InterruptedException {
        Payment[] payments = new Payment[]{
                ...
                new Payment("4539 796 98 8528", "05/1/2020", "€8,04"),
                ...
                new Payment("4716 578 37 7779", "26/4/2019", "€4,09"),
        ...
        System.out.println("Summe Mit Loop: " + summeMitLoop(payments));
        System.out.println("Summe Mit Rekursion: " + summeMitRekursion(payments, 0));
        System.out.println("Doppelt Mit Loops: ");
        doppeltMitLoop(payments);
        Thread.sleep(100);
        System.out.println("Doppelt Mit Rekursion: ");
        doppeltMitRekursion(payments, 0, new ArrayList<>());
        System.out.println("Datum Mit Loop: ");
        try {
            datumMitLoop(payments, new SimpleDateFormat("dd/MM/yyyy").parse("10/4/2019"), new SimpleDateFormat("dd/MM/yyyy").parse("20/4/2019"));
        ...
        System.out.println("Datum Mit Loop: ");
        try {
            datumMitRekursion(payments, new SimpleDateFormat("dd/MM/yyyy").parse("10/4/2019"), new SimpleDateFormat("dd/MM/yyyy").parse("20/4/2019"),0);
        } catch (ParseException e) {
            e.printStackTrace();
        }
    }

    public static double summeMitLoop(Payment[] payments) {
        double sum = 0;

        for (Payment payment : payments) {
            String amount = payment.getAmount().replace("€", "").replace(",", ".");
    ...
    public static double summeMitRekursion(Payment[] payments, int index) {
        Payment payment = payments[index];
        String amount = payment.getAmount().replace("€", "").replace(",", ".");
        double dAmount = Double.parseDouble(amount);
        if (index + 1 == payments.length) return dAmount;
        return summeMitRekursion(payments, index + 1) + dAmount;
    }

    public static void doppeltMitLoop(Payment[] payments) {
        List<String> numbers = new ArrayList<>();

        for (Payment payment : payments) {
            String nr = payment.getCardnumber().replace(" ", "");
            if (numbers.contains(nr)) {
                System.err.println(nr + " already in use");
            } else {
                numbers.add(nr);
            }
        }
    ...
    public static void doppeltMitRekursion(Payment[] payments, int index, List<String> numbers) {
        Payment payment = payments[index];
        ...
        if (index + 1 == payments.length) return;
        doppeltMitRekursion(payments, index + 1, numbers);
    }

    public static void datumMitLoop(Payment[] payments, Date front, Date end) {
        for (Payment payment : payments) {
            String sDate = payment.getDate();
            try {
                Date date = new SimpleDateFormat("dd/MM/yyyy").parse(sDate);
                if (date.after(front) && date.before(end)) {
    ...
    public static void datumMitRekursion(Payment[] payments, Date front, Date end, int index) {
        Payment payment = payments[index];
        ...
        if (index + 1 == payments.length) return;
        datumMitRekursion(payments, front, end, index + 1);
